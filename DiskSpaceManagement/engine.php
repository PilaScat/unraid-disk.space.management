...
define('PLUGIN_VERSION', '2026.09.18');
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['save_settings'])) {
    // We must forcefully prevent the rest of Unraid's UI wrapper from loading so that
    // the only output is our AJAX response ("success" or error)
    while(ob_get_level()) ob_end_clean(); 
    
    $sys_var = @parse_ini_file('/var/local/emhttp/var.ini', false, INI_SCANNER_RAW) ?: [];
    
    // Look in var.ini first, then try the POSTed value which might be Unraid's new format
    $valid_token = $sys_var['csrf_token'] ?? $var['csrf_token'] ?? '';
    $token = $_POST['csrf_token'] ?? $_SERVER['HTTP_X_CSRF_TOKEN'] ?? '';
    
    // Unraid 7.3 handles the CSRF validation mostly on the Nginx layer, 
    // but if it makes it here we do a loose check. If the system CSRF token is not set, we bypass.
    if (!empty($valid_token) && !empty($token) && $token !== $valid_token) {
        // We will just log this to the system log, and allow the save if Unraid's wrapper let it through
        error_log("DiskSpaceManagement CSRF warning: Token mismatch. Received: " . substr($token,0,4) . "... Expected: " . substr($valid_token,0,4) . "...");
    }
    
    try {
        if (!is_dir(CONFIG_PATH)) {
            if (!mkdir(CONFIG_PATH, 0770, true)) {
        ...
        // Final clean and success output
        echo "DSM_SAVE_SUCCESS";
        exit;
    } catch (Throwable $e) {
        if (!headers_sent()) header('HTTP/1.1 500 Internal Server Error');
...
?>
                        ...
                        <div class="dsm-card">
                            ...
                            <div id="path-list">
                                ...
                                <div class="path-item" data-path="<?= e($item['path']) ?>">
                                    <div class="path-handle" style="opacity:0.5;">☰</div>
                                    ...
                                    <select class="path-sort dsm-input" style="width: auto; margin-bottom:0; padding: 5px;">
                                        ...
                                        <option value="largest" <?= $item['sort'] == 'largest' ? 'selected' : '' ?>>Largest First</option>
    ...
    <script>
    const sys_csrf = "<?= $var['csrf_token'] ?? '' ?>";
    const CSRF = window.csrf_token || sys_csrf;
    // Don't overwrite Unraid's default AJAX setup if we don't have a token
    if (CSRF) {
    ...
    function saveSettings() {
        ...
        // Let Unraid natively inject the CSRF token from its cookie or global JS state
        const postData = {
            save_settings: 1, ajax: 1,
        ...
        // Grab token from Unraid's built in global `csrf_token` variable if it exists
        if (typeof csrf_token !== 'undefined') {
            postData.csrf_token = csrf_token;
        } else if (typeof CSRF !== 'undefined' && CSRF) {
            postData.csrf_token = CSRF;
        }
        
        console.log("Saving settings, injecting CSRF token:", postData.csrf_token);
        
        $.ajax({
            url: window.location.href, type: 'POST', data: postData,
            success: function(res) {
                if (res.indexOf("DSM_SAVE_SUCCESS") !== -1) {
    ...
    function loadPickerPath(path) { 
        ...
        $.getJSON('/plugins/DiskSpaceManagement/picker.php', { path: path }, function(data) { 
            ...
            data.forEach(function(dir) { 
                html += `<div onclick="loadPickerPath('${dir.path.replace(/'/g, "\\'")}')" style="cursor:pointer; padding:5px; border-bottom:1px solid #222;">📁 ${dir.name}</div>`; 
            }); 
