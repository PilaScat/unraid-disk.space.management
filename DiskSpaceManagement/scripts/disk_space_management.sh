        ...
        while true; do
            ...
            for entry in "${PATH_ARRAY[@]}"; do
                ...
                if [ -n "$item" ]; then
                    ...
                    #
                    # BUG FIX: the item is added to SIMULATED_PROCESSED_PATHS and
                    # 'continue 2' restarts the while loop, so the next folder of the
                    # same path is selected instead of falling through to the next path.
                    # ----------------------------------------------------------------
...
if [ "$send_notification" = true ]; then
    ...
    importance="normal"
    [ "$ANY_DISK_BELOW_THRESHOLD" = true ] && [ "$MOVE_COUNT" -eq 0 ] && importance="warning"

    # Send notification via Unraid's notify system
    # -e = event type, -s = subject, -d = short description (one line, sent as-is to agents), -m = full message (\n separated), -i = importance
    /usr/local/emhttp/plugins/dynamix/scripts/notify -e "Disk Space Management" -s "Run Summary" -d "$description" -m "$message" -i "$importance"
fi
